...
termux_step_configure() {
	...
	# Use bundled gn
	export PATH="$TERMUX_PKG_SRCDIR/buildtools/linux64:$PATH"

	# Use python2 as the default python
	rm -rf $TERMUX_PKG_TMPDIR/python2-bin
	mkdir -p $TERMUX_PKG_TMPDIR/python2-bin
	ln -s $(command -v python2) $TERMUX_PKG_TMPDIR/python2-bin/python
	export PATH="$TERMUX_PKG_TMPDIR/python2-bin:$PATH"
...
" > $_common_args_file
	...
	if [ "$TERMUX_ARCH" = "i686" ]; then
		...
		export BUILD_CFLAGS="--target=i386-linux-gnu"
		export BUILD_CPPFLAGS=""
		export BUILD_CXXFLAGS="--target=i386-linux-gnu"
		export BUILD_LDFLAGS="--target=i386-linux-gnu"
	...
	if [ "$TERMUX_ARCH" = "x86_64" ]; then
		...
		pushd $TERMUX_PKG_TMPDIR/host-toolchain
		sed "s|@COMPILER@|$(command -v clang-13)|" $TERMUX_PKG_BUILDER_DIR/wrapper-compiler.in |
			sed "s|@HOST_TRIPLE@|x86_64-linux-gnu|" |
			sed "s|@NEW_SYSROOT@|$_sysroot_path|;s|@TERMUX_PREFIX@|$TERMUX_PREFIX|" > ./wrapper_cc
		sed "s|@COMPILER@|$(command -v clang++-13)|" $TERMUX_PKG_BUILDER_DIR/wrapper-compiler.in |
			sed "s|@HOST_TRIPLE@|x86_64-linux-gnu|" |
			sed "s|@NEW_SYSROOT@|$_sysroot_path|;s|@TERMUX_PREFIX@|$TERMUX_PREFIX|" > ./wrapper_cxx
