...
electron_add_conflicts() {
    # Revert patches
	shopt -s nullglob
	local f
	for f in $(basename "$TERMUX_PKG_BUILDER_DIR")/electron-*; do
		echo "[INFO]: Found $f"
		local f_basename="$(basename $f)"
		if [ "$f_basename" != "electron-$TERMUX_PKG_VERSION" ]; then
			echo "[INFO]: Add $f_basename"
			TERMUX_PKG_CONFLICTS+=", $f_basename"
		fi
	done
	shopt -u nullglob
}
...
termux_step_configure() {
	...
	rm -rf $TERMUX_PREFIX/lib/electron
